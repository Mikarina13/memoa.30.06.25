...
export class MemoirIntegrations {
  ...
  static async getGalleryItems(userId: string, memoriaProfileId?: string): Promise<any[]> {
    try {
      ...
      if (memoriaProfileId) {
        ...
        // Fix: Use JSON.stringify only for the JSONB metadata field, not for the text[] tags field
   ...
   * @param isCelebrity Whether this is a celebrity tribute
   */
   ...
   */
  static async updateMemoriaProfile(profileId: string, profileData: any): Promise<MemoriaProfile> {
    try {
      const { data, error } = await supabase
        .from('memoria_profiles')
        .update({
  ...
  static async upload3DModelFile(userId: string, file: File, memoriaProfileId?: string): Promise<string> {
    try {
      const folder = memoriaProfileId ? `memoria/${memoriaProfileId}/models` : `memoir/${userId}/models`;
      const filename = `${folder}/${Date.now()}-${file.name}`;
      
      return await this.uploadFile(userId, file, 'models', filename);
    } catch (error) {
